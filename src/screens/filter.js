...
const Filter = (props) => {
  ...
  return (
    <View
      style={{height: '100%', width: '100%', marginTop: 15, paddingBottom: 15}}>
      <View
        style={{
          width: '100%',
          display: 'flex',
      ...
      <ScrollView
        ...
        <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
          <Text
            style={{width: 40, marginLeft: 10, color: 'white', fontSize: 14}}>
            {low}
          </Text>
          <Text
            style={{width: 40, marginLeft: 10, color: 'white', fontSize: 14}}>
        ...
        {filterOptions.length !== 0 ? (
          filterOptions.filter_data.map((i, k) => (
            <View key={k}>
              <View>
                ...
                {i.filter_option.map((filterData, index) => {
                  return (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (
                          selectedSubCategory[filterData.custom_field_id] &&
                          selectedSubCategory[
                            filterData.custom_field_id
                          ].includes(filterData.value)
                        ) {
                          const filterValues = flattenDeep(
                            values(selectedSubCategory),
                          );
                          const filterReturn = filterValues.filter(
                            (item) => item !== filterData.value,
                          );

                          setSelectedSubCategory({
                            ...selectedSubCategory,
                            [filterData.custom_field_id]: filterReturn,
                          });
                        } else if (
                          selectedSubCategory[filterData.custom_field_id]
                        ) {
                          setSelectedSubCategory({
                            ...
                            ...selectedSubCategory[
                              filterData.custom_field_id
                            ].push(filterData.value),
                          });
                        } else {
                          setSelectedSubCategory({
                            ...selectedSubCategory,
                            [filterData.custom_field_id]: [filterData.value],
                          });
                        }
                      }}>
                      <SmallLGBtn
